...
local function StopNoClip()
    ...
    SetPlayerFallDistance(PlayerId(), 9000.0)
    SetPedCanRagdoll(ped, true)
    local entityHeightAboveGround = GetEntityHeightAboveGround(ped)
    CreateThread(function()
        while entityHeightAboveGround > 1.0 do
            entityHeightAboveGround = GetEntityHeightAboveGround(ped)
            Wait(100)
        end
        Wait(750)
        SetEntityInvincible(ped, false)
        SetPlayerFallDistance(PlayerId(), -1)
    end)
