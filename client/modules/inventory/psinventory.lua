...
function OpenStash(stashName, owner)
    if not Config.NewQBInventory then
        ...
        TriggerServerEvent("ps-inventory:server:OpenInventory", "stash", stashName, other)
...
function OpenTrunk(vehicle, plate)
    if not Config.NewQBInventory then
        ...
        TriggerServerEvent("ps-inventory:server:OpenInventory", "trunk", plate, other)
        TriggerEvent("ps-inventory:client:SetCurrentTrunk", plate)
    else
...
function OpenGlovebox(plate)
    if not Config.NewQBInventory then
        ...
        TriggerServerEvent("ps-inventory:server:OpenInventory", "glovebox", plate, other)
        TriggerEvent("ps-inventory:client:SetCurrentGlovebox", plate)
    else
...
function openJobStash(data)
    if not Config.NewQBInventory then
        ...
        TriggerServerEvent("ps-inventory:server:OpenInventory", "stash", data.jobStashName, other)
        TriggerEvent("ps-inventory:client:SetCurrentStash", data.jobStashName)
    else
...
RegisterNetEvent("snipe-menu:client:openinventory", function(otherPlayer)
    if not Config.NewQBInventory then
        ...
        TriggerServerEvent("ps-inventory:server:OpenInventory", "otherplayer", otherPlayer)
    else
